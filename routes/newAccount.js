...
router.post('/', (req, res)=>{
    ...
    mongoDb.db.users.find((err,doc)=>{
        ...
        if(doc.length > 10) res.json({status: 200, updated: "FAILED", message: "ACCOUNT_LIMIT_EXCEEDED"})
        else{
            const obj = {
                userName,
                email,
            ...
            else res.json({status: 400, updated: "FAILED", message: "ALREADY_EXISTING_ACCOUNT"});
        }
    });
});
